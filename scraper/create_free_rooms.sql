WITH
  ...
  busy_sorted AS (
    SELECT
      ...
      COUNT(*)      OVER (PARTITION BY bi.room, bi.weekday) ORDERS cnt
    FROM busy_intervals AS bi
  ),
  ...
  missing_days AS (
    SELECT
      p.room,
      p.weekday
    FROM room_week_pairs AS p
    LEFT JOIN (
      SELECT DISTINCT room, weekday FROM busy_intervals
    ) AS bi
